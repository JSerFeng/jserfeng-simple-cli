...
const init = async (projectName, options) => {
  ...
  await installDep(redux, router, axios, ts)
...
function installDep(options) {
  ...
  return new Promise((resolve, reject) => {
    const {code} = shell.exec('yarn add dart-sass -D')
    if (code === 0) {
      resolve()
    } else {
      reject()
    ...
    res => {
      if (axios || router || redux)
        shell.exec(`yarn add ${axios ? 'axios' : ''} ${router ? 'react-router-dom' : ''} ${redux ? 'redux react-redux' : ''} ${ts ? '@types/react-redux' : ''}`)
    },
    err => {
      console.log('意外退出')
      console.log(code)
